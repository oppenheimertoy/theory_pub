...
**Сложное ПО. Введение и основы ООП.**
## Лекция 1 - Введение
...
![](/ssrpo/general_theory/pics/Parallel-vs-sequential.jpg)
...
int main() {
    ...
    // immutableObj.setValue("New Value");
...
### 19. Дайте определение понятия «шаблон проектирования». Назовите плюсы и минусы шаблонов проектирования. Опишите назначение ШП Property container.

Про Property Container - [тут](#property-container---контейнер-атрибутов).
...
int main() {
    ...
    container.setProperty("name", std::string("MyObject"));
    container.setProperty("id", 123);

    if (container.hasProperty("name")) {
        std::cout << "Name: " << std::any_cast<std::string>(container.getProperty("name")) << std::endl;
    }

    if (container.hasProperty("id")) {
        std::cout << "ID: " << std::any_cast<int>(container.getProperty("id")) << std::endl;
    }
...
После этого мы можем просто унаследоваться от PropertyContainer и получить функционал, который позволяет изменять атрибутивный состав в рантайме.

```cpp
class Vehicle : public PropertyContainer {
private:
    std::string model;
    std::string make;

public:
    Vehicle(const std::string& model, const std::string& make) : model(model), make(make) {}

    void printDetails() const {
        std::cout << "Model: " << model << ", Make: " << make << std::endl;
...
int main() {
    Vehicle myCar("Model S", "Tesla");
    myCar.setProperty("Year", 2021);
    myCar.setProperty("Color", std::string("Red"));

    myCar.printDetails(); // This will print model, make, and all dynamic properties

    return 0;
}
```

### 20. Дайте определение понятия «шаблон проектирования». Назовите плюсы и минусы шаблонов проектирования. Опишите назначение ШП Builder.

Описание шаблона Builder [тут](#builder---строитель).
...
public:
    ...
    void display() const {
        std::cout << "Car: " << make << " " << model << ", Year: " << year << ", Color: " << color << std::endl;
    }
};
...
Во всех задачах нужно - построить модель описанной ниже предметной области, выделить сущности, значения, службы и агрегат.

Думаю стоит лишний раз пробежаться по тому, что хочет увидеть Фетисов. Например понятие агрегата в DDD не самая тривиальная штука, имхо.
...
А вот так про агрегат пишет сам Эванс:
...
Если пытаться как то упростить то, что он имеет в виду, то это наверное будет звучать как то так - сущность это некоторый объект предметной области нашей задачи, который имеет некоторый "поток"(жизненный цикл) в рамках нашей предметной области, связанный с его идентичностью(aka уникальными свойствами), а не своими атрибутами. Т.е от сущность транспотрного средства - это объект предметной области, основная уникальная способность которого перемещаться из точки A в точку B. А то как это просиходит, сколько у данной сущности колес и какой мотор, нас уже не волнует - это проблема конкретной реализации.

Далее зависит от предметной области и формулировки. Многие задачи из экзамена встречались на РК или разбирались на семах. 

### Необходимо определить три морских судна, находящихся на минимальном прямом расстоянии от другого судна, которое терпит бедствие и посылает сигналы SOS.

- Сущности: Ключевыми сущностями являются DistressSignal, RescueOperation, и Vessel. Каждая из этих сущностей представляет важные элементы системы мониторинга и спасения.

- Значения: Атрибуты классов, такие как name, location и signalType, представляют собой данные, которые система использует для выполнения своих функций.

- Службы: Методы, такие как sendSOS(), calculateDistance(), initiateRescue(), и navigate(), представляют собой службы, которые выполняют определенные действия в системе.

- Агрегат - Объединяет все службы и сущности для обеспечения эффективного реагирования на чрезвычайные ситуации на море.
Отвечает за прием и обработку сигналов SOS.
Координирует действия по определению и направлению ближайших судов к месту бедствия.

Ниже представлена диаграмма

![](./pics/exam_tasks/sos_vessels.svg)
