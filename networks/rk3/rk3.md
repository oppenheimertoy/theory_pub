...
Тут будут описаны те вопросы, которые нельзя включить в одну из тем, в силу того, что они рассматривают более специфические темы или напрямую не связаны с реализацией протоколов.
...
### Вопросы про внутренне устройство сетевых устройств

Таблица описывающая различные типы памяти сетевых устройств

| Память       | Энергозависимый / энергонезависимый | Хранлища                                      |
|--------------|------------------------------------|----------------------------------------------|
| ОЗУ          | Энергозависимый                    | - Текущая выполняемая копия IOS              |
|              |                                    | - Файл текущей конфигурации                  |
|              |                                    | - IP-маршрутизация и таблицы ARP             |
|              |                                    | - Буфер пакета                               |
| ПЗУ          | Энергонезависимый                  | - Инструкции по загрузке                     |
|              |                                    | - Основное программное обеспечение для диагностики |
|              |                                    | - Ограниченная версия IOS                    |
| NVRAM        | Энергонезависимый                  | - Файл загрузочной конфигурации              |
| Флеш-память  | Энергонезависимый                  | - IOS                                        |
|              |                                    | - Прочие системные файлы                     |

### Назовите предназначение Flash-памяти в коммутаторе/маршрутизаторе

В общем случае хранит операционную систему сетевого устройства, а также все необходимое программное обеспечение

### При выходе из строя Flash-памяти что именно загрузит маршрутизатор

ПЗУ - постоянное запоминающее устройство. Реализовано непосредственно на интегральной плате. С него и будет загруженна некоторая версия операционной системы по умолчанию.


### На каком этапе загрузки маршрутизатор определяет факт выхода из строя Flash-памяти

Поиск и загрузка программного обеспечения IOS.
...
- Номер протокола 89;
- Данный протокол является [протоколом динамической маршрутизации](#динамическая-маршрутизация), а именно [протоколом внутреннего шлюза](#две-группы-протоколов-динамической-маршрутизации); 
...
Также можно настроить RID командой, где X.X.X.X - RID в формате IPv4   
```
...
##### Настройка статичсекого маршрута в зоне OSPF

Данная настройка нужна для связи разных [автономных сетей](#две-группы-протоколов-динамической-маршрутизации). 

Итак, есть роутер на гринице АС, который мы хотим связать статически с другой АС. В нашем случае мы настраиваем Router0, так как он внутри АС, в которой настроен протокол OSPF.

![](./OSPF_AS.png)

Для настройки нужно зайти в Router0 и написать данные команды
```
Router0(config)#ip route 0.0.0.0 0.0.0.0 10.15.10.1
```

Данной командой мы прописываем в таблицу маршрутизации статический маршрут. Он говорит, что IP [0.0.0.0/0](https://en.wikipedia.org/wiki/0.0.0.0) должен отправляется по адресу 10.15.10.1, который в свою очередь является IP на интрефесе Router1.

0.0.0.0 - неопредённый адрес. То есть, если внутри AS нет такого адреса, запрос будет пересылаться по дефолтному статическому маршруту в другую АС.

Теперь надо сообщить данный дефолтный маршрут остальным роутерам внутри АС. Для этого на Роутере 0 пропишем команды
```
...
**Примечание**
В других протоколах сообщение дефолтного маршрута остальным роутерам прописывается другой командой, например в EIGRP командой
```
Router1(config)#router eigrp 1
Router1(config-router)#redistribute static
```

Чтобы удалить информацию о дефолтном маршруте в таблице маршрутизации, нужно прописать команду на роутере, **который изначально разослал всем информацию**, в данном случае на Роутере 0
```
Router0(config-router)#no default-information originate
```
